...
def main():
    ...
    # 3rd CZ 
    state.gate[[4,5]] = move.Move(state.storage[[1,2]]) 
    state = move.GlobalCZ(atom_state=state) 
    state.storage[[1,2]] = move.Move(state.gate[[4,5]]) # move back 

    # rz -3/4 pi
    state.gate[[7]] = move.Move(state.storage[[1]])
    state = move.LocalRz(atom_state=state,phi=(-3/4)*pi,indices=[7])
    state.storage[[1]] = move.Move(state.gate[[7]])
    ...
    #  CZ 
    state.gate[[2,3]] = move.Move(state.storage[[0,2]]) 
    state = move.GlobalCZ(atom_state=state) 
    state.storage[[0,2]] = move.Move(state.gate[[2,3]]) # move back

    #  CZ 
    state.gate[[2,3]] = move.Move(state.storage[[0,1]]) 
    state = move.GlobalCZ(atom_state=state) 
    state.storage[[0,1]] = move.Move(state.gate[[2,3]]) # move back
    ...
    # Rx: pi
    state.gate[[2]] = move.Move(state.storage[[1]])
    state = move.LocalXY(atom_state=state, x_exponent=pi,indices=[2], 
                         axis_phase_exponent=0.0)
    state.storage[[1]] = move.Move(state.gate[[2]])

    # Rx: pi/4
    state.gate[[3]] = move.Move(state.storage[[2]])
    state = move.LocalXY(atom_state=state, x_exponent=(1/4)*pi,indices=[3], 
                         axis_phase_exponent=0.0)
    state.storage[[2]] = move.Move(state.gate[[3]])
