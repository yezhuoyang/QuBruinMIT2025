...
def main():
    ...
    state = move.LocalXY(atom_state=state,x_exponent=-0.5,axis_phase_exponent=0.5,indices=[3,4])
    state.storage[[1]] = move.Move(state.gate[[1]])

    state.gate[[1,2]] = move.Move(state.storage[[1,2]]) # Move for entangling gate
    state = move.GlobalCZ(atom_state=state) # CZ gate
    state.storage[[1,2]] = move.Move(state.gate[[1,2]]) # Move back from entangling gate
...
if __name__ == "__main__":
    ...
    with open("assets/qasm/2.qasm", "r", encoding="utf-8") as file:
        content = file.read()
