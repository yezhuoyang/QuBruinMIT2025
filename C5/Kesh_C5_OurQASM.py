...
def main():
    ...
    # Move entangling pairs (S[3]=G[0], S[4]=G[1]), (S[1]=G[2], S[5]=G[3]), (S[2]=G[4], S[6]=G[5])
    state.gate[[0]] = move.Move(state.storage[[3]])
    state.gate[[1]] = move.Move(state.storage[[4]])
    ...
    # Now apply rest of CZ gates
    state = move.GlobalCZ(atom_state=state)
    # Here the assignement is: S[3]=G[0], S[4]=G[1], S[1]=G[2], S[5]=G[3], S[2]=G[4], S[6]=G[5]

    # Move back all qubits except S[6]=G[5] and S[4]=G[1], so, S[3]=G[0], S[1]=G[2], S[5]=G[3], S[2]=G[4]
    state.storage[[4]] = move.Move(state.gate[[1]])
    state.storage[[1]] = move.Move(state.gate[[2]])
    state.storage[[5]] = move.Move(state.gate[[3]])
    state.storage[[2]] = move.Move(state.gate[[4]])

    # Move entangling pairs S[0]=G[0], S[4]=G[1] and second pair is S[1]=G[4], S[6]=G[5]
    state.gate[[1]] = move.Move(state.storage[[0]])
    state.gate[[4]] = move.Move(state.storage[[1]])

    # Now apply rest of CZ gates
    state = move.GlobalCZ(atom_state=state)
    # Here the assignement is: S[3]=G[0], S[4]=G[1], S[1]=G[4], S[6]=G[5]
    ...
    state = move.LocalXY(state, -0.5*pi, -0.5*pi, indices=[2, 3,4]) #S[1]=G[4], S[2]=G[2], S[3]=G[0]
    state = move.GlobalXY(state, pi, 0.0)
    state = move.LocalXY(state, -pi, 0.0, indices=[2,3,4])
